...
public class NeuralNetwork {
    ...
    public NeuralNetwork() {
        IntStream.range(0, NUMB_OF_INPUT_NEURONS).forEach(i -> neurons[i] = new Neuron(Math.random() - 0.5,
                new double[] { Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5 }, LayerType.I));
        IntStream.range(NUMB_OF_INPUT_NEURONS, NUMB_OF_INPUT_NEURONS + NUMB_OF_HIDDEN_NEURONS)
                .forEach(i -> neurons[i] = new Neuron(Math.random() - 0.5,
                        new double[] { Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5 }, LayerType.H));
        neurons[NUMB_OF_NEURONS - 1] = new Neuron(Math.random() - 0.5,
                new double[] { Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5 }, LayerType.H);
    }

    public NeuralNetwork(Neuron[] neurons) {
        IntStream.range(0, neurons.length).forEach(i -> {
            this.neurons[i] = new Neuron(neurons[i].getThreshold(), neurons[i].getWeights(), neurons[i].getLayerType());
        });
    }
