    ...
    public static void main(String[] args) throws IOException {
        ...
        while (true) {
            ...
            if (command.equals("exit")) {
                ...
                new SimulatedAnnealing().anneal(neuralNetwork);
            } else if (command.equals("run")) {
                double[] result = new double[] { 0, 0, 0, 0 };
                for (int i = 0; i < TRAINING_DATA.length; i++) {
                    result[i] = neuralNetwork.run(TRAINING_DATA[i][0]);
                }
                printResult(result);
            }
        }
    ...
    public static void printResult(double[] result) {
        ...
        IntStream.range(0, TRAINING_DATA.length).forEach(i -> {
            IntStream.range(0, TRAINING_DATA[0][0].length)
                    .forEach(j -> System.out.print("    " + TRAINING_DATA[i][0][j] + "      |   "));
            System.out.print("  " + TRAINING_DATA[i][1][0] + "  | " + String.format("%.10f", result[i]) + " \n");
        });
    }
